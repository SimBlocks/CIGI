...
bool CLineOfSightSegmentRequestGeodeticToEntityExtendedHandler::Handle()
{
  if (g_CigiLibGlobals.pEventMessenger == nullptr)
  {
    if (g_CigiLibGlobals.pLogger != nullptr)
    {
      g_CigiLibGlobals.pLogger->LogError("CLineOfSightSegmentRequestGeodeticToEntityExtendedHandler::Handle: IGCigiLib event messenger is not initialized.");
    }
    return false;
  }
...
const sbio::cigi::SLineOfSightRequest& CLineOfSightSegmentRequestGeodeticToEntityExtendedHandler::GetRequest() const
{
  return m_Request;
}

sbio::cigi::SLineOfSightRequest& CLineOfSightSegmentRequestGeodeticToEntityExtendedHandler::GetRequestRef()
{
  return m_Request;
}

bool CLineOfSightSegmentRequestGeodeticToEntityExtendedHandler::ResolveStartPoint(GeocentricCoordinates& point)
{
  point = ConvertCigiGeodeticToWorldCoordinates(m_Request.sourceGeodeticCoordinates);
  return true;
}

bool CLineOfSightSegmentRequestGeodeticToEntityExtendedHandler::ResolveEndPoint(GeocentricCoordinates& point)
{
